...
async def fwd_media(m_msg: Message) -> Optional[Message]:
    try:
        ...
        try:
            return await m_msg.copy(chat_id=Var.BIN_CHANNEL) # Retry once after FloodWait
...
# Helper function to create link buttons
...
@require_token
@force_channel_check
async def link_handler(bot: Client, msg: Message):
    # Validate user
    ...
    if num_files == 1:
        await process_single(bot, msg, msg.reply_to_message, status_msg)
    else:
        await process_batch(bot, msg, msg.reply_to_message.id, num_files, status_msg)
...
@require_token
@force_channel_check
async def private_receive_handler(bot: Client, msg: Message):
    ...
    await process_single(bot, msg, msg, status_msg)
...
async def channel_receive_handler(bot: Client, msg: Message):
    ...
    try:
        ...
        # Try to add buttons if admin
        if await is_admin(bot, msg.chat.id):
            try:
...
async def process_single(bot: Client, msg: Message, file_msg: Message, status_msg: Message):
    try:
        # Copy file
        ...
        if str(e):  # Only notify if there's an actual error message
...
async def process_batch(bot: Client, msg: Message, start_id: int, count: int, status_msg: Message):
    processed = 0
    failed = 0
    links_list = []
    ...
    for batch_start in range(0, count, 10):
        ...
        try:
            ...
            await asyncio.sleep(e.value + 1)
            messages = await bot.get_messages(msg.chat.id, batch_ids) # Retry once
        except Exception as e:
    ...
    for i in range(0, len(links_list), 20):
        ...
        if i + 20 < len(links_list):  # Only sleep if more chunks coming
