...
def str_to_int_set(val: str) -> set[int]:
    ...
    for x in val.split():
        try:
            ...
            # collect-all-errors: junk tokens surface as one error, not N
            bad.append(x)
            continue
    if bad:
...
class Var:
    ...
    # above it surface to tg_call's bounded retry. Keep well below
    ...
    # per-client admission cap for the streaming server.
    MAX_CONCURRENT_STREAMS: int = _get_int("MAX_CONCURRENT_STREAMS", "8", min_val=1)

    # touch-buffer flush cadence + cap.
