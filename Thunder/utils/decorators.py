    ...
    async def wrapper(client, message: Message, *args, **kwargs):
        try:
            ...
            if not token_data or 'token' not in token_data:
                logger.error(f"Failed to generate token for user {user_id} in require_token.")
                await message.reply_text("Sorry, could not generate an access token. Please try again later.", quote=True)
                return
            ...
            logger.info(f"Sent token request to user {user_id} for {func.__name__}.")
            return
        except Exception as e:
    ...
    async def wrapper(client, callback_query):
        ...
        try:
            if _cached_owner_ids is None:
                ...
                _cached_owner_ids = [owner_ids_config] if isinstance(owner_ids_config, int) else list(owner_ids_config)
