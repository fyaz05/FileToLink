...
class OuoIoPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        response = await asyncio.to_thread(
            self.session.get, f"http://ouo.io/api/{api_key}?s={url}"
        )
...
class CuttLyPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        response = await asyncio.to_thread(
            self.session.get, f"http://cutt.ly/api/api.php?key={api_key}&short={url}"
        )
        if response.status_code == 200:
...
class GenericShortenerPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        response = await asyncio.to_thread(
            self.session.get, f"https://{self.domain}/api?api={api_key}&url={quote(url)}"
        )
        if response.status_code == 200:
