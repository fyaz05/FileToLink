...
class OuoIoPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        def _do_request():
            return self.session.get(f"http://ouo.io/api/{api_key}?s={url}")
        response = await asyncio.to_thread(_do_request)
...
class CuttLyPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        def _do_request():
            return self.session.get(f"http://cutt.ly/api/api.php?key={api_key}&short={url}")
        response = await asyncio.to_thread(_do_request)
        if response.status_code == 200:
...
class GenericShortenerPlugin(ShortenerPlugin):
    ...
    async def shorten(self, url: str, api_key: str) -> str:
        def _do_request():
            return self.session.get(f"https://{self.domain}/api?api={api_key}&url={quote(url)}")
        response = await asyncio.to_thread(_do_request)
        if response.status_code == 200:
...
class ShortenerSystem:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            await asyncio.to_thread(self._init_scraper)
