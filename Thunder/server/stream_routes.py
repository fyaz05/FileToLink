...
# Updated regex: allow underscores and hyphens in the secure hash.
...
def optimal_client_selection():
    ...
    # Select the client with the minimal current workload.
...
async def track_workload(client_id):
    """
    Async context manager to track workload:
    Increments workload when entering, and decrements when leaving.
    """
...
async def status_endpoint(_):
    ...
    return web.json_response({
        ...
        "total_workloads": sum(work_loads.values()),
...
async def media_delivery(request: web.Request):
    # Select a client with the lowest current workload.
    client_id, client = optimal_client_selection()
    # Use the context manager to update the workload count.
    async with track_workload(client_id):
        path = request.match_info["path"]
        message_id, secure_hash = parse_media_request(path, request.query)
        # Pass the selected client info to the streaming handler.
